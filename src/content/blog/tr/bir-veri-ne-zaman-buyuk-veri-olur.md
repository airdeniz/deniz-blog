...
Buraya kadar çizgiyi hep "tek makinenin sınırı" üzerinden çektik. Ama burada ince ama önemli
bir düzeltme gerekiyor: **bir veriye büyük veri diyebilmek için onun illa birden fazla
bilgisayar (bir dağıtık sistem / cluster) tarafından işlenmesi şart değildir.** Çünkü büyük
veri **verinin kendi karakteristiğidir**; dağıtık sistemler ise o veriyi işlemek için
kullanılan **çözümlerden yalnızca biridir.** İkisi çok sık birbirine karıştırılır.

Peki neden tarihte hep yan yana yazıldılar? 2000'lerin başında tek bir bilgisayarın işlemci
(CPU) ve bellek (RAM) kapasitesi hem sınırlı hem çok pahalıydı. Veri dünyası bir anda
patlayınca şirketler bu devasa yükü tek makineye sığdıramadı — **scale-up duvarına** çarptılar.
İşte tam o noktada Google, **MapReduce** makalesiyle ve ardından **Hadoop**, dünyaya şunu dedi:
*"Devasa tek bir bilgisayar almaya çalışmayın; 500 tane sıradan bilgisayarı birleştirip tek bir
makine gibi çalıştırın"* (**scale-out**). Büyük veri ile dağıtık sistemlerin adı bu yüzden
tarihe hep birlikte geçti.

Ama bugünün donanımı o Hadoop döneminden çok farklı bir yerde. Artık tek bir sunucunun (single
node) içine terabaytlarca RAM, yüzlerce işlemci çekirdeği ve saniyede gigabaytlarca okuyan NVMe
SSD'ler koyulabiliyor. Bu şu demek: eskiden 50 bilgisayarlık bir Hadoop cluster'ı kurarak
çözülen ve "büyük veri" denen birçok problem, bugün tek bir güçlü bulut sunucusunda — in-memory
veritabanları ya da modern, optimize edilmiş execution engine'lerle — üstelik ağ (network)
gecikmesi olmadan çok daha hızlı işlenebiliyor.

Yani "tek makine sınırı" sabit bir duvar değil, **donanımla birlikte sürekli kayan bir
çizgidir.** Daha da önemlisi: bir veri tek makinenin hafızasına sığıp orada işlenebiliyor olsa
bile, hâlâ inanılmaz karmaşık, hızlı ve yapılandırılmamışsa o veri **yine büyük veridir.**
Araçlar amaca hizmet eder; mimariyi belirleyen şey verinin dayattığı zorunluluklardır — dağıtık
sistem, bu zorunlulukların en bilinen cevabıdır, tanımı değil.
...
veritabanları) verinin hacmi, hızı ya da karmaşıklığı altında ezilmeye başladığı, sizi başka
araçlara — ister dağıtık bir kümeye, ister tek ama devasa bir düğüme — geçmeye mecbur bıraktığı
an, işte tam o an büyük veri çizgisini geçmişsinizdir. Gösterişli tek bir veri türünü arka plandaki makine egzozundan ayıran da,
